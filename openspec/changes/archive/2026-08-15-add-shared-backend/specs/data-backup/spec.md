...
`average_exclusions`, and no `isArchived`/`sortOrder` columns for categories. Verified against the
user's own export (`expense-tracker-export-2026-08-14.csv`, 1768 rows): it has exactly `row_index,
date, category, amount_eur, note, is_daily, source_sheet` and nothing else. Anything relying on
that CSV as a full-fidelity export or migration input silently loses month flags, average
exclusions (the user has real ones recorded for July 2026), and category archive/order state — the
app keeps working and keeps showing plausible numbers, just wrong ones, with nothing indicating the
shift. This capability adds a full-fidelity JSON export/import to the current Dexie app that closes
that gap, built and shipped before any other part of the shared-backend migration.
