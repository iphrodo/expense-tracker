...
The application SHALL open directly on the expense entry screen on launch, regardless of device.
The app SHALL NOT show a dashboard, month list, or any other screen before entry.

#### Scenario: Cold launch lands on entry
- **WHEN** the user opens the installed app (or loads the app in a browser) with no prior
  in-app navigation
- **THEN** the expense entry screen is the first and only screen rendered

### Requirement: Amount field is focused on mount and after every save
The amount input SHALL receive focus automatically when the entry screen mounts, and SHALL
receive focus again immediately after each successful save, so the user never reaches for the
mouse or trackpad to begin the next entry. On devices that present a numeric keyboard, mounting
or refocusing the amount field SHALL present it.

#### Scenario: No interaction needed to start typing on mount
- **WHEN** the entry screen finishes mounting
...
On save, the system SHALL update the UI (clear the form back to an empty entry state, per the
focus requirement above) without waiting for the IndexedDB write to complete, and SHALL show a
toast with an Undo action for a few seconds. The system SHALL NOT show a confirmation dialog,
navigate away from the entry screen, or show a separate success screen.

#### Scenario: Form clears immediately without waiting on storage
- **WHEN** the user saves
- **THEN** the entry screen returns to its empty initial state and a toast with an Undo action
...
- **WHEN** the user saves
- **THEN** no confirmation dialog is shown and no navigation occurs away from the entry screen
