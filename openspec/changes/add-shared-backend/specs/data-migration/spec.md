...
Moves the data already accumulated in the app's local IndexedDB — the authoritative source, since
it holds everything entered since the original seed import — into the shared Postgres database
exactly once, with verification strong enough that nothing local is discarded on a guess.

The migration's input is the `data-backup` capability's JSON export, not the per-transaction CSV
export: the CSV has no columns for `month_flags` or `average_exclusions` and is verified lossy for
this purpose (see the `data-backup` spec's Purpose). The user re-exporting with the `data-backup`
exporter is a blocking manual prerequisite (see the "Migration input" requirement below), and is
ordered before schema/client work in `tasks.md` precisely so this input is available when the load
script is needed.
